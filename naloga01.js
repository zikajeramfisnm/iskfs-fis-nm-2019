...
io.sockets.on("connection", function(socket) {
    socket.on("ukazArduinu", function(štUkaza) {
        ...
        if (štUkaza == "5") {
            board.digitalWrite(5, board.HIGH); // zapišemo +5V na pin 5
        }
        if (štUkaza == "4") {
            board.digitalWrite(5, board.LOW); // zapišemo 0V na pin5
        }
        if (štUkaza == "7") {
            board.digitalWrite(3, board.HIGH); // zapišemo +5V na pin 3
        }
        if (štUkaza == "6") {
            board.digitalWrite(3, board.LOW); // zapišemo 0V na pin3
        }
        if (štUkaza == "9") {
            board.digitalWrite(13, board.HIGH); // zapišemo +5V na pin 13
            board.digitalWrite(8, board.HIGH); // zapišemo +5V na pin 8
